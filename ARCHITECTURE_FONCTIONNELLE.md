...
>
> **État de référence :** fin de session 6 (EPIC-1 clos).
...
|---|---|---|
| Liste de l'utilisateur | Firestore `schedules/{uid}` | temps réel au load, sauvegarde debouncée (1s) à chaque modif |
| Statut / épisodes / broadcast | Jikan `/anime/{id}` | synchro batch au boot, TTL 6 h par anime |
| Relations (séquelles) | Jikan `/anime/{id}/relations` | cache IDB persistant |
| Recommandations liées | Jikan `/anime/{id}/recommendations` | cache IDB TTL 7 j |
| Saison en cours / à venir | Jikan `/seasons/*` | cache localStorage TTL 24 h |
