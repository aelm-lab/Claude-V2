...
**Dernier numéro attribué : DEC-189.** 
...
ID	Décision	Pourquoi ça casse si on l'ignore
DEC-187	🔴 Un mock de test doit reproduire tous les champs que le code de production filtre, pas seulement ceux qu'il affiche. Avant d'écrire ou d'étendre une fixture, relire les continue / if (...) return de la fonction consommatrice et couvrir chacun	relationsBody du helper omettait type: 'ANIME'. useAniListApi.ts:446 écarte tout node dont type !== 'ANIME' → le helper était incapable de produire une seule relation depuis sa création (AUD-57). Une spec configurant relations: {...} recevait une liste vide et testait l'écran vide en croyant tester l'écran plein. Le champ n'est jamais affiché : aucune assertion ne pouvait le révéler
DEC-188	🔴 Format de réponse au PO — 4 blocs, pas plus. 📍 où on en est (1 ligne) · 👤 impact utilisateur (1-2 lignes) · 🔜 prochaine US anticipée · ⏭️ actions numérotées. Le raisonnement technique va dans l'US ou dans les documents, jamais dans le chat. Sur incident : 2 lignes maximum (« ça bloque parce que X → conséquence Y »). Le détail technique n'est produit que sur demande explicite du PO	Le PO est fonctionnel et non technique. Il a demandé l'arrêt des explications techniques à plusieurs reprises avant que la règle soit écrite. Coût mesuré en SE-073 : la capacité de conversation a été consommée à 85 % pour 2 US livrées, dont une part majoritaire en démonstrations que le PO ne lisait pas. Une explication non demandée n'est pas de la pédagogie, c'est de la dette de contexte
| **DEC-189** | 🔴 **Toute US touchant une surface visible par l'utilisateur est précédée d'une maquette validée par le PO.** Pas de maquette validée, pas d'US. La maquette montre l'état actuel et les options, avec le coût de chacune (dont les specs E2E qu'elle casse). Le PO tranche sur la maquette, jamais sur la description textuelle | Une description en prose d'un changement visuel n'est pas vérifiable : le PO valide un texte et découvre un écran. Cas fondateur SE-074 — l'option « une seule rangée » semblait évidemment supérieure en prose ; la maquette a rendu visible qu'elle enterrait 5 actions derrière un menu, la semaine de l'ouverture bêta |
