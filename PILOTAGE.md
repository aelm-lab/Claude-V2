...
| **H7** | **Plafond : 250 lignes par document de Knowledge.** Au-delà → scinder ou archiver. C'est la contrainte de recherche sémantique, pas une préférence. *`AGENTS.md` est exempté (lecture linéaire par Gemini, qui n'a aucune autre source) et plafonné à **350 lignes** — au-delà, c'est le signe qu'une règle y a été dupliquée* | Continu | Claude alerte |
